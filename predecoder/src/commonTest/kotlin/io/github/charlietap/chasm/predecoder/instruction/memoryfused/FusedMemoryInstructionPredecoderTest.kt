...
class FusedMemoryInstructionPredecoderTest {
    ...
    fun `predecodes memory size into frame slots without using caches`() {
        ...
        val instruction = FusedMemoryInstruction.MemorySize(
            destination = FusedDestination.FrameSlot(0),
            memoryIndex = Index.MemoryIndex(0),
        )

        val dispatchable = FusedMemoryInstructionPredecoder(context, instruction).value
