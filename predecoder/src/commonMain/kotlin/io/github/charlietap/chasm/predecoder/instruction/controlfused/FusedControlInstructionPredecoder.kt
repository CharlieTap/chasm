...
import io.github.charlietap.chasm.runtime.instruction.FusedControlInstruction as RuntimeFusedControlInstruction
...
internal fun FusedControlInstructionPredecoder(
    ...
    when (instruction) {
        is FusedControlInstruction.BrIf -> strictBrIfInstruction(context, instruction).bind()
        is FusedControlInstruction.BrOnCast -> strictBrOnCastInstruction(context, instruction).bind()
        is FusedControlInstruction.BrOnCastFail -> strictBrOnCastFailInstruction(context, instruction).bind()
        is FusedControlInstruction.BrOnNonNull -> strictBrOnNonNullInstruction(context, instruction).bind()
        is FusedControlInstruction.BrOnNull -> strictBrOnNullInstruction(context, instruction).bind()
        is FusedControlInstruction.BrTable -> strictBrTableInstruction(context, instruction).bind()
        is FusedControlInstruction.Call -> strictCallInstruction(context, instruction).bind()
        is FusedControlInstruction.CallIndirect -> strictCallIndirectInstruction(context, instruction).bind()
        is FusedControlInstruction.CallRef -> strictCallRefInstruction(instruction)
        is FusedControlInstruction.If -> strictIfInstruction(context, instruction).bind()
        is FusedControlInstruction.ReturnCall -> strictReturnCallInstruction(context, instruction).bind()
        is FusedControlInstruction.ReturnCallIndirect -> strictReturnCallIndirectInstruction(context, instruction).bind()
        is FusedControlInstruction.ReturnCallRef -> strictReturnCallRefInstruction(instruction)
        is FusedControlInstruction.Throw -> strictThrowInstruction(instruction)
        is FusedControlInstruction.ThrowRef -> strictThrowRefInstruction(instruction)
    }
}
...
private fun strictReturnCallInstruction(
    context: PredecodingContext,
    instruction: FusedControlInstruction.ReturnCall,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
    val address = context.instance.functionAddress(instruction.functionIndex).bind()
...
private fun strictCallIndirectInstruction(
    context: PredecodingContext,
    instruction: FusedControlInstruction.CallIndirect,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
...
private fun strictCallRefInstruction(
    instruction: FusedControlInstruction.CallRef,
...
private fun strictReturnCallIndirectInstruction(
    context: PredecodingContext,
    instruction: FusedControlInstruction.ReturnCallIndirect,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
    val address = context.instance.tableAddress(instruction.tableIndex).bind()
    val table = context.store.table(address)
    val type = context.instance.runtimeTypes[instruction.typeIndex.idx]
...
private fun strictReturnCallRefInstruction(
    instruction: FusedControlInstruction.ReturnCallRef,
): DispatchableInstruction {
    val functionSlot = strictControlOperandSlot(instruction.functionReference)
        ?: error("control fused instruction must be frame-slot lowered to immediate/frame-slot shapes before predecode")
...
private fun strictThrowRefInstruction(
    instruction: FusedControlInstruction.ThrowRef,
): DispatchableInstruction {
