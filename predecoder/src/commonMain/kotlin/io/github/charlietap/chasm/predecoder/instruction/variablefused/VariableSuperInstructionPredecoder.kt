...
import io.github.charlietap.chasm.runtime.instruction.VariableSuperInstruction as RuntimeFusedVariableInstruction

internal fun VariableSuperInstructionPredecoder(
    context: PredecodingContext,
    instruction: VariableSuperInstruction,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
    when (instruction) {
        is VariableSuperInstruction.GlobalGet -> GlobalGetSuperInstructionPredecoder(context, instruction).bind()
        is VariableSuperInstruction.GlobalSet -> GlobalSetSuperInstructionPredecoder(context, instruction).bind()
        is VariableSuperInstruction.LocalSet -> LocalSetSuperInstructionPredecoder(context, instruction)
        is VariableSuperInstruction.LocalTee -> unsupportedUnloweredVariableInstruction()
    }
}

private fun GlobalGetSuperInstructionPredecoder(
    context: PredecodingContext,
    instruction: VariableSuperInstruction.GlobalGet,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
...
private fun GlobalSetSuperInstructionPredecoder(
    context: PredecodingContext,
    instruction: VariableSuperInstruction.GlobalSet,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
    val global = resolveGlobal(context, instruction.globalIdx).bind()
    strictGlobalSetInstruction(context, instruction.operand, global)
}

private fun LocalSetSuperInstructionPredecoder(
    context: PredecodingContext,
    instruction: VariableSuperInstruction.LocalSet,
