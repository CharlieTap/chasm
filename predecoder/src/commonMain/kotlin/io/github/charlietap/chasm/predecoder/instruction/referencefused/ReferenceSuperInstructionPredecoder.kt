...
import io.github.charlietap.chasm.runtime.instruction.ReferenceSuperInstruction as RuntimeFusedReferenceInstruction

internal fun ReferenceSuperInstructionPredecoder(
    context: PredecodingContext,
    instruction: ReferenceSuperInstruction,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
    when (instruction) {
        is ReferenceSuperInstruction.RefEq -> strictRefEqInstruction(instruction)
        is ReferenceSuperInstruction.RefIsNull -> strictRefIsNullInstruction(instruction)
        is ReferenceSuperInstruction.RefAsNonNull -> strictRefAsNonNullInstruction(instruction)
        is ReferenceSuperInstruction.RefFunc -> strictRefFuncInstruction(context, instruction).bind()
        is ReferenceSuperInstruction.RefNull -> strictRefNullInstruction(instruction)
        is ReferenceSuperInstruction.RefTest -> strictRefTestInstruction(context, instruction)
        is ReferenceSuperInstruction.RefCast -> strictRefCastInstruction(context, instruction)
    }
}

private fun strictRefEqInstruction(
    instruction: ReferenceSuperInstruction.RefEq,
...
private fun strictRefIsNullInstruction(
    instruction: ReferenceSuperInstruction.RefIsNull,
): DispatchableInstruction {
...
private fun strictRefNullInstruction(
    instruction: ReferenceSuperInstruction.RefNull,
): DispatchableInstruction {
...
private fun strictRefFuncInstruction(
    context: PredecodingContext,
    instruction: ReferenceSuperInstruction.RefFunc,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
...
private fun strictRefTestInstruction(
    context: PredecodingContext,
    instruction: ReferenceSuperInstruction.RefTest,
): DispatchableInstruction {
...
private fun strictRefCastInstruction(
    context: PredecodingContext,
    instruction: ReferenceSuperInstruction.RefCast,
): DispatchableInstruction {
    val referenceSlot = strictReferenceOperandSlot(instruction.reference)
    val destinationSlot = strictReferenceDestinationSlot(instruction.destination)
