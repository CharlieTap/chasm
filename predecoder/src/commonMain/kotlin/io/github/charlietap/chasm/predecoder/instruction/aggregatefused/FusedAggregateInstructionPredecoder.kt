...
import io.github.charlietap.chasm.runtime.instruction.FusedAggregateInstruction as RuntimeFusedAggregateInstruction
...
internal fun FusedAggregateInstructionPredecoder(
    ...
    when (instruction) {
        is FusedAggregateInstruction.ArrayCopy -> strictArrayCopyInstruction(instruction)
        is FusedAggregateInstruction.ArrayFill -> strictArrayFillInstruction(instruction)
        is FusedAggregateInstruction.ArrayGet -> strictArrayGetInstruction(instruction)
        is FusedAggregateInstruction.ArrayGetSigned -> strictArrayGetSignedInstruction(instruction)
        is FusedAggregateInstruction.ArrayGetUnsigned -> strictArrayGetUnsignedInstruction(instruction)
        is FusedAggregateInstruction.ArrayLen -> strictArrayLenInstruction(instruction)
        is FusedAggregateInstruction.ArrayNew -> strictArrayNewInstruction(context, instruction).bind()
        is FusedAggregateInstruction.ArrayNewDefault -> strictArrayNewDefaultInstruction(context, instruction).bind()
        is FusedAggregateInstruction.ArrayNewData -> strictArrayNewDataInstruction(context, instruction).bind()
        is FusedAggregateInstruction.ArrayNewElement -> strictArrayNewElementInstruction(context, instruction).bind()
        is FusedAggregateInstruction.ArrayNewFixed -> strictArrayNewFixedInstruction(context, instruction).bind()
        is FusedAggregateInstruction.ArraySet -> strictArraySetInstruction(instruction)
        is FusedAggregateInstruction.ArrayInitData -> strictArrayInitDataInstruction(context, instruction).bind()
        is FusedAggregateInstruction.ArrayInitElement -> strictArrayInitElementInstruction(context, instruction).bind()
        is FusedAggregateInstruction.RefI31 -> strictRefI31Instruction(instruction)
        is FusedAggregateInstruction.I31GetSigned -> strictI31GetSignedInstruction(instruction)
        is FusedAggregateInstruction.I31GetUnsigned -> strictI31GetUnsignedInstruction(instruction)
        is FusedAggregateInstruction.AnyConvertExtern -> strictAnyConvertExternInstruction(instruction)
        is FusedAggregateInstruction.ExternConvertAny -> strictExternConvertAnyInstruction(instruction)
        is FusedAggregateInstruction.StructGet -> strictStructGetInstruction(instruction)
        is FusedAggregateInstruction.StructGetSigned -> strictStructGetSignedInstruction(instruction)
        is FusedAggregateInstruction.StructGetUnsigned -> strictStructGetUnsignedInstruction(instruction)
        is FusedAggregateInstruction.StructNew -> strictStructNewInstruction(context, instruction).bind()
        is FusedAggregateInstruction.StructNewDefault -> strictStructNewDefaultInstruction(context, instruction).bind()
        is FusedAggregateInstruction.StructSet -> strictStructSetInstruction(instruction)
    }
}
