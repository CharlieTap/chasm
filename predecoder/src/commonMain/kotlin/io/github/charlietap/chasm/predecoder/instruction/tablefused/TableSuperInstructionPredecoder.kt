...
import io.github.charlietap.chasm.runtime.instruction.TableSuperInstruction as RuntimeFusedTableInstruction

internal fun TableSuperInstructionPredecoder(
    context: PredecodingContext,
    instruction: TableSuperInstruction,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
    when (instruction) {
        is TableSuperInstruction.TableCopy -> strictTableCopyInstruction(context, instruction).bind()
        is TableSuperInstruction.TableFill -> strictTableFillInstruction(context, instruction).bind()
        is TableSuperInstruction.TableGet -> strictTableGetInstruction(context, instruction).bind()
        is TableSuperInstruction.TableGrow -> strictTableGrowInstruction(context, instruction).bind()
        is TableSuperInstruction.TableInit -> strictTableInitInstruction(context, instruction).bind()
        is TableSuperInstruction.TableSet -> strictTableSetInstruction(context, instruction).bind()
        is TableSuperInstruction.TableSize -> strictTableSizeInstruction(context, instruction).bind()
    }
}
...
private fun strictTableSizeInstruction(
    context: PredecodingContext,
    instruction: TableSuperInstruction.TableSize,
): Result<DispatchableInstruction, ModuleTrapError> = binding {
    val table = resolveTable(context, instruction.tableIdx).bind()
    val destinationSlot = strictTableDestinationSlot(instruction.destination) ?: return@binding unsupportedUnloweredTableInstruction()
    TableSizeDispatcher(
