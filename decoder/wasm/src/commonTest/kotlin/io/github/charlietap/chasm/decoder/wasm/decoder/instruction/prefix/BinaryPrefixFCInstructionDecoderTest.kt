...
class BinaryPrefixFCInstructionDecoderTest {
    ...
    fun `can decode prefixed numeric instructions`() {
        ...
        opcodeMap.forEach { (opcode, instruction) ->
            val actual = BinaryPrefixFCInstructionDecoder(
                reader = reader,
                opcode = opcode,
    ...
    fun `returns an unknown prefix instruction error when the opcode doesn't match`() {
        ...
        val reader = FakeWasmBinaryReader()
        val actual = BinaryPrefixFCInstructionDecoder(reader, opcode)
